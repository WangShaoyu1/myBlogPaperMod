...
let allModelsList = await readJsonFilesFromFolder('../followers');
...
async function storeDifLevelDate(level) {
    return new Promise(async (resolve, reject) => {
        ...
        await writeToFile(JSON.stringify({followers: handledModelsList}, null, 2), `./output/juejin/followerRank/gte_${level}_followers.json`)
            .then(() => {
                console.log(`gte_${level}_followers.json written successfully`)
                resolve();
            });
    })
}
