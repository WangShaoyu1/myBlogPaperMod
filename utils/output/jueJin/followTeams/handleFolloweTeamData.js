...
let allModelsList = await readJsonFilesFromFolder('../followTeams');
...
async function storeDifLevelDate(level) {
    return new Promise(async (resolve, reject) => {
        ...
        await writeToFile(JSON.stringify({followers: handledModelsList}, null, 2), `./output/juejin/followerRank/gte_${level}_followTeam.json`)
            .then(() => {
                console.log(`gte_${level}_followTeam.json written successfully`)
                resolve();
            });
    })
}
