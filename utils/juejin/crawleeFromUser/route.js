...
router.addHandler('DETAIL', async ({page, request, enqueueLinks, log}) => {
    ...
    weight++
    writeToFile(`${request.url}\n`, `./output/juejin/followerRank/combineData/visitedUrls.txt`, true).then(() => console.log('combineGteData-visitedUrls.txt written successfully'));
...
router.addDefaultHandler(async ({request, page, enqueueLinks, log}) => {
    ...
    let allUrls = (await readJsonFilesFromFolder('../output/juejin/followerRank/combineGteData'))[0].urls,
        visitedUrls = await readFile('../output/juejin/followerRank/combineGteData/visitedUrls.txt'),
